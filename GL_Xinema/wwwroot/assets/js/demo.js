...
(function($ , undefined) {
	...
	sidebarMatchings.default = {
		...
		'footer': 'bgc-white-tp2 brc-primary-m3',
	...
	sidebarMatchings.dark = jQuery.extend({}, sidebarMatchings.default,
		{
			...
			'user-img': 'brc-secondary-m2',
			'user-info': 'text-blue-l2 text-center',
			'user-desc': 'text-white-tp3 text-80',

			'footer': 'bgc-white-tp1 brc-transparent',
	...
	sidebarMatchings.green = jQuery.extend({}, sidebarMatchings.steelblue,
		{
			...
			'badge': 'border-0 bgc-white-tp1 text-dark-tp1',
	...
	function BoxedLayout(box) {
		...
		if(box == 'none') {
			$('.body-container, .navbar-inner, .page-content').removeClass('container');
			$('.navbar-inner > .container').contents().unwrap();
		}
		else if(box == 'all') {
			$('.page-content').removeClass('container');
			$('.body-container, .navbar-inner').addClass('container');
			if( !window.isAceLayout2 ) {
				$('.navbar-inner > .container').contents().unwrap();
			}
			else {
			   if( $('.navbar-inner > .container').length == 0 ) $('.navbar-inner').wrapInner('<div class="container" />');    
			}
		}
		else if(box == 'not-navbar') {
			if( window.isAceLayout2 ) return;//not applicable to second layout
			$('.navbar-inner, .page-content').removeClass('container');
			$('.body-container').addClass('container');        
			$('.navbar-inner').wrapInner('<div class="container" />');
		}
